				 ...
				 struct sk_buff *skb, u8 flags)
{
	...
	if (virtio_net_hdr_tnl_to_skb(skb, &hdr->hdr, vi->rx_tnl_offset,
	...
	if (skb->encapsulation && flags & VIRTIO_NET_HDR_F_DATA_VALID) {
		/* hdr parsing cleared the ip_summed status; the stack
		 * expect CHECKSUM_PARTIAL for gso packets, but the
		 * later gro stage will set reset the ip_summed and
		 * checks field as needed.
		 */
		skb->ip_summed = CHECKSUM_UNNECESSARY;
		if (flags & VIRTIO_NET_HDR_F_UDP_TUNNEL_CSUM)
			skb->csum_level = 1;
	}
...
static int xmit_skb(struct send_queue *sq, struct sk_buff *skb, bool orphan)
{
	...
	if (virtio_net_hdr_tnl_from_skb(skb, &hdr->hdr, vi->tx_tnl_offset,
...
static int virtnet_probe(struct virtio_device *vdev)
{
	...
	if (virtio_has_feature(vdev, VIRTIO_NET_F_GUEST_UDP_TUNNEL_GSO))
		vi->rx_tnl_offset = vi->hdr_len;
	if (virtio_has_feature(vdev, VIRTIO_NET_F_HOST_UDP_TUNNEL_GSO))
		vi->tx_tnl_offset = vi->hdr_len;
	if (vi->tx_tnl_offset || vi->rx_tnl_offset)
