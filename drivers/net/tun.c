			    ...
			    int noblock, bool more)
{
	...
	if (tun->flags & IFF_VNET_HDR) {
		...
		hdr_len = __tun_vnet_hdr_get(vnet_hdr_sz, flags, from, gso);
		if (hdr_len < 0)
			return hdr_len;
	...
	if (tun_vnet_hdr_tnl_to_skb(flags, skb, &hdr)) {
			    ...
			    struct iov_iter *iter)
{
	...
	if (vnet_hdr_sz) {
		...
		/*
		 * In case of inconsistent hdr size and (tunnel) offloads
		 * strips the latter
		 */
		if (unlikely(vnet_hdr_sz < parsed_size))
			flags &= ~TUN_VNET_TNL_MASK;

		ret = tun_vnet_hdr_tnl_from_skb(flags, tun->dev, skb, &hdr);
		if (ret)
			return ret;

		parsed_size = tun_vnet_parse_size(flags);
		gso = (struct virtio_net_hdr *)&hdr;
...
build:
	...
	if (tun_vnet_hdr_tnl_to_skb(flags, skb, tnl_hdr)) {
		atomic_long_inc(&tun->rx_frame_errors);
		kfree_skb(skb);
		ret = -EINVAL;
		goto out;
